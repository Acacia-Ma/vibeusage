...
async function main() {
  ...
  const legacySrcDir = path.join(rootDir, "insforge-src", "functions");
  const esmSrcDir = path.join(rootDir, "insforge-src", "functions-esm");
  const outDir = path.join(rootDir, "insforge-functions");

  const legacyEntryPoints = await listEntryPoints(legacySrcDir);
  const esmEntryPoints = await listEntryPoints(esmSrcDir);
  const allEntryPoints = [...legacyEntryPoints, ...esmEntryPoints];
  if (allEntryPoints.length === 0) {
    throw new Error(
      `No entry points found under ${path.relative(rootDir, legacySrcDir)}/ or ${path.relative(rootDir, esmSrcDir)}/`,
    );
  }
  assertNoDuplicateEntryNames(allEntryPoints);

  if (!check) {
    await removeStaleOutputs({ outDir, entryPoints: allEntryPoints });
  }

  const esbuild = require("esbuild");

  const outputFiles = [];
  if (legacyEntryPoints.length > 0) {
  ...
  if (!check) {
    process.stdout.write(
      `Built ${allEntryPoints.length} InsForge edge functions into ${path.relative(rootDir, outDir)}/\n`,
    );
    return;
  }
