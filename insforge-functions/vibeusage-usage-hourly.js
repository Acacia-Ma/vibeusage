...
var CORE_KEY13 = "__vibeusageUsageRollupCore";
...
var vibeusage_usage_hourly_default = withRequestLogging2("vibeusage-usage-hourly", async function(request, logger) {
  ...
  if (isUtcTimeZone2(tzContext)) {
    ...
    let rowCount2 = 0;
    const { error: error2 } = await forEachPage3({
      createQuery: () => {
      ...
      onPage: (rows) => {
        ...
        for (const row of pageRows) {
          ...
          if (hasModelFilter2) {
            ...
            const dateKey = extractDateKey2(ts) || dayLabel;
  ...
  const { error } = await forEachPage3({
    ...
    onPage: (rows) => {
      ...
      for (const row of pageRows) {
        ...
        if (hasModelFilter) {
          const rawModel = normalizeUsageModel2(row?.model);
          const dateKey = extractDateKey2(ts) || dayKey;
